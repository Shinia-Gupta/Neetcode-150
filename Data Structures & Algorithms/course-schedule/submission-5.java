class Solution {
    public boolean canFinish(int numCourses, int[][] prerequisites) {
        ...
        return canFinishAll(graph,nodeToPrereqMap,queue,numCourses);
    ...
    private boolean canFinishAll(List<List<Integer>> graph,Map<Integer,Integer> map,Queue<Integer> queue,int numCourses)
    {
        int count=0;
        while(!queue.isEmpty()){
            int node=queue.poll();
            ...
            for(int neighbor:graph.get(node)){
                if(map.containsKey(neighbor)){
                    ...
                    if(map.get(neighbor)==0){
                        queue.add(neighbor);
                        // map.remove(neighbor);        // not needed
                    }
                }
            }
        }
