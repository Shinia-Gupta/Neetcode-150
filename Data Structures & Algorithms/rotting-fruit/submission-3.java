...
int minutes=0;
//i was getting this issue that when all the fruits have been processed to rotten and there is no fresh fruit left, the while loop still ran for that last cell of the queue which was added as rotten, so at the end, minutes got increased by 1 in the output
//so i added a consition that if fresh fruits are still present across the grid somewhere, only then the while loop should continue running.
